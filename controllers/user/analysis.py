...
def analyze_company(symbol, company_name, is_yfinance):
    try:
        ...
        if df.empty:
            return jsonify({'error': f"No data available for the date range {st} to {ed}."}), 400
        ...
        return jsonify({'error': f"An error occurred while analyzing the company: {e}"}), 500
...
def compare_companies(symbol1, symbol2, companies):
    try:
        ...
        if df1.empty:
            return jsonify({'error': f"No data available for {symbol1} in the date range {st} to {ed}."}), 400

        if df2.empty:
            return jsonify({'error': f"No data available for {symbol2} in the date range {st} to {ed}."}), 400
        ...
        return jsonify({'error': f"An error occurred while comparing the companies: {e}"}), 500
